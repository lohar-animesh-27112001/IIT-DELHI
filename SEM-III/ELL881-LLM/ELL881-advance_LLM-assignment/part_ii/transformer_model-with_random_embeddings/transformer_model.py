...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, Q, K, V, mask=None, kv_cache=None):
        ...
        if kv_cache is not None:
            # Use cached keys and values for generation
...
class Transformer(nn.Module):
    def __init__(self, vocab_size, d_model, num_layers, num_heads, d_ff, max_seq_length=512, dropout=0.1, fasttext_embeddings=None):
        ...
        if d_model % num_heads != 0:
            ...
            print(f"Adjusted d_model to {d_model} to be divisible by {num_heads}")
...
def main():
    ...
    # Model configuration
    d_model = 296  # Adjusted to be divisible by 8
    num_layers = 3
    num_heads = 8
    d_ff = 1024
    max_seq_length = 64
    ...
    for i in range(min(5, len(predictions))):
        print(f"Prompt: {references[i][:50]}...")
        print(f"Generated: {predictions[i][:50]}...")
        print("---")
