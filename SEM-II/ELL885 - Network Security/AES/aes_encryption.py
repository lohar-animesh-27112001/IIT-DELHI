from PIL import Image
import numpy as np
import hashlib

class AESEncryptor:
    SBOX = [
        ,
    ]

    RCON = [
        0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36
    ]

    def __init__(self, key):
        self.key = self._pad_key(key)
        self.nk = len(self.key) // 4
        self.nr = {4:10, 6:12, 8:14}[self.nk]
        self.round_keys = self._key_expansion()

    def _pad_key(self, key):
        hash_obj = hashlib.sha256(key.encode()).digest()
        lengths = {16:16, 24:24, 32:32}
        return hash_obj[:lengths.get(len(key), 32)]

    def _key_expansion(self):
        key_words = [self.key[i:i+4] for i in range(0, len(self.key), 4)]
        for i in range(self.nk, 4*(self.nr+1)):
            temp = key_words[i-1]
            if i % self.nk == 0:
                temp = self._sub_word(self._rot_word(temp))
                temp = bytes([temp[0] ^ self.RCON[i//self.nk-1]] + list(temp[1:]))
            elif self.nk > 6 and i % self.nk == 4:
                temp = self._sub_word(temp)
            new_word = bytes([a ^ b for a, b in zip(key_words[i-self.nk], temp)])
            key_words.append(new_word)
        return [key_words[i*4:(i+1)*4] for i in range(len(key_words)//4)]

    def _rot_word(self, word):
        return word[1:] + word[0:1]

    def _sub_word(self, word):
        return bytes([self.SBOX[b] for b in word])

    def _bytes_to_state(self, data):
        return [list(data[i:i+4]) for i in range(0, 16, 4)]

    def _state_to_bytes(self, state):
        return bytes(sum(state, []))

    def _sub_bytes(self, state):
        return [[self.SBOX[b] for b in row] for row in state]

    def _shift_rows(self, state):
        return [
            state[0],
            state[1][1:] + state[1][0:1],
            state[2][2:] + state[2][0:2],
            state[3][3:] + state[3][0:3]
        ]

    def _mix_columns(self, state):
        mixed = []
        for col in range(4):
            s = [
                state[0][col],
                state[1][col],
                state[2][col],
                state[3][col]
            ]
            mixed.append([
                self._gf_mul(0x02, s[0]) ^ self._gf_mul(0x03, s[1]) ^ s[2] ^ s[3],
                s[0] ^ self._gf_mul(0x02, s[1]) ^ self._gf_mul(0x03, s[2]) ^ s[3],
                s[0] ^ s[1] ^ self._gf_mul(0x02, s[2]) ^ self._gf_mul(0x03, s[3]),
                self._gf_mul(0x03, s[0]) ^ s[1] ^ s[2] ^ self._gf_mul(0x02, s[3])
            ])
        return [[row[i] for row in mixed] for i in range(4)]

    def _gf_mul(self, a, b):
        p = 0
        for _ in range(8):
            if b & 1:
                p ^= a
            a <<= 1
            if a & 0x100:
                a ^= 0x11b
            b >>= 1
        return p

    def _add_round_key(self, state, round_key):
        return [[b ^ k for b, k in zip(row, key)] 
                for row, key in zip(state, round_key)]

    def _encrypt_block(self, block):
        state = self._bytes_to_state(block)
        state = self._add_round_key(state, self.round_keys[0])
        for round in range(1, self.nr):
            state = self._sub_bytes(state)
            state = self._shift_rows(state)
            state = self._mix_columns(state)
            state = self._add_round_key(state, self.round_keys[round])
        state = self._sub_bytes(state)
        state = self._shift_rows(state)
        state = self._add_round_key(state, self.round_keys[self.nr])
        return self._state_to_bytes(state)

    def encrypt_image(self, image_path, output_path):
        img = Image.open(image_path)
        img_array = np.array(img)
        original_shape = img_array.shape
        data = img_array.tobytes()
        padded_data = self._pad_data(data)
        encrypted = bytearray()
        for i in range(0, len(padded_data), 16):
            block = padded_data[i:i+16]
            encrypted += self._encrypt_block(block)
        encrypted_img = Image.frombytes(img.mode, img.size, bytes(encrypted))
        encrypted_img.save(output_path)
        return encrypted_img

    def _pad_data(self, data):
        """Add PKCS#7 padding"""
        pad_len = 16 - (len(data) % 16)
        return data + bytes([pad_len] * pad_len)

if __name__ == "__main__":
    key = "my_secret_key"
    encryptor = AESEncryptor(key)

    input_image = "images/plain_image.png"
    output_image = "images/encrypted_image.png"
    encrypted_img = encryptor.encrypt_image(input_image, output_image)
    encrypted_img.show()